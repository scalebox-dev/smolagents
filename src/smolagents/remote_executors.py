...
__all__ = ["E2BExecutor", "DockerExecutor", "WasmExecutor","ScaleboxExecutor"]
...
class ScaleboxExecutor(RemotePythonExecutor):
    """
    Executes Python code using Scalebox.
    ...
    def __init__(self, additional_imports: list[str], logger, **kwargs):
        ...
        try:
            ...
            raise ModuleNotFoundError(
                """Please install 'scalebox' extra to use ScaleboxExecutor: `pip install scalebox`"""
            )
        kwargs["timeout"]=1800
        self.sandbox = Sandbox.create(**kwargs)
        self.python_context=self.sandbox.create_code_context(
            language="python",
            cwd="/tmp"
        )
        self.installed_packages = self.install_packages(additional_imports)
        self.logger.log("Scale is running", level=LogLevel.INFO)

    def run_code_raise_errors(self, code: str) -> CodeOutput:
        execution = self.sandbox.run_code(code,context=self.python_context)
        execution_logs = "\n".join([str(log) for log in execution.logs.stdout])
    ...
    def cleanup(self):
        """Clean up the Scalebox sandbox and resources."""
        try:
            if hasattr(self, "sandbox"):
                self.logger.log("Shutting down sandbox...", level=LogLevel.INFO)
                self.sandbox.kill()
